...
class NeRF2_Runner():
    ...
    def __init__(self, mode, dataset_type, **kwargs) -> None:
        ...
        if kwargs_train['load_ckpt'] or mode in ('test', 'synthesize'):
        ...
        ## Dataset
        if mode == 'synthesize':
            self.logger.info("Loading synthesize dataset (all positions, no train/test split)...")
            synthesize_cls = dataset_dict['synthesize']
            tx_pos_file = kwargs_path.get('synthesize_tx_pos_file', 'labels_split_ant.csv')
            self.synthesize_set = synthesize_cls(self.datadir, self.scale_worldsize, tx_pos_file=tx_pos_file)
            self.synthesize_iter = DataLoader(self.synthesize_set, batch_size=self.batch_size, shuffle=False, num_workers=0)
            self.logger.info("Synthesize set: %d positions x %d gateways = %d total samples",
                             self.synthesize_set.n_pos, self.synthesize_set.n_gateways, len(self.synthesize_set))
        else:
        ...
        while self.current_iteration <= self.total_iterations:
            with tqdm(total=len(self.train_iter), desc=f"Iteration {self.current_iteration}/{self.total_iterations}") as pbar:
                for train_input, train_label in self.train_iter:
                    ...
                    elif self.dataset_type in ('ble', 'absolute_channel', 'a_b'):
                        tx_o, rays_o, rays_d = train_input[:, :3], train_input[:, 3:6], train_input[:, 6:]
                        predict_rssi_real, predict_rssi_imag = self.renderer.render_channel(tx_o, rays_o, rays_d)
                        loss = sig2mse(predict_rssi_real, train_label[:,0]) + sig2mse(predict_rssi_imag, train_label[:,1])
    ...
    def eval_network_rssi(self):
        ...
        with torch.no_grad():
            with open(os.path.join(self.logdir, self.expname, "result.txt"), 'w') as f:
                for i, (test_input, test_label) in enumerate(self.test_iter):
                    test_input, test_label = test_input.to(self.devices), test_label.to(self.devices)
                    ...
                    predict_rssi_real, predict_rssi_imag = self.renderer.render_channel(tx_o, rays_o, rays_d)
                    gt_rssi = test_label
                    error_real = abs(predict_rssi_real - gt_rssi[:,0])
                    error_imag = abs(predict_rssi_imag - gt_rssi[:,1])
                    error_total = error_real + error_imag
                    self.logger.info(f"gt{i} - Median error:%.5f", torch.median(error_total))
        ...
        with torch.no_grad():
            with open(os.path.join(self.logdir, self.expname, "result.txt"), 'w') as f:
                for test_input, test_label in self.test_iter:
                    ...
                    error_imag = abs(predict_rssi_imag - gt_rssi[:,1])
                    error_total = error_real + error_imag
                    self.logger.info("Median error:%.5f", torch.median(error_total))
                    
                    # write predicted RSSI values to file
                    for i, rssi in enumerate(predict_rssi_real):
                        f.write("{:.2f}, {:.2f}".format(predict_rssi_real[i].item(), predict_rssi_imag[i].item()) + '\n')

        # result = np.loadtxt(os.path.join(self.logdir,self.expname, "result.txt"), delimiter=",")
        # self.logger.info("Total Median error:%.2f", np.median(abs(result[:,0] - result[:,1])))

    def synthesize(self):
        """Generate synthetic channel data for all positions in labels_split_ant.csv
        and save to a CSV file.

        Output format: one row per TX position, columns interleaved as
          real_g01, imag_g01, real_g02, imag_g02, ..., real_gN, imag_gN
        so 2 * n_gateways columns total (6 columns for 3 gateways).
        """
        self.logger.info("Start synthesizing channel data for %d positions x %d gateways",
                         self.synthesize_set.n_pos, self.synthesize_set.n_gateways)
        self.nerf2_network.eval()

        n_pos = self.synthesize_set.n_pos
        n_gateways = self.synthesize_set.n_gateways
        total = n_pos * n_gateways

        all_real = torch.zeros(total)
        all_imag = torch.zeros(total)

        with torch.no_grad():
            offset = 0
            for synth_input, _ in tqdm(self.synthesize_iter, desc="Synthesizing"):
                synth_input = synth_input.to(self.devices)
                tx_o = synth_input[:, :3]
                rays_o = synth_input[:, 3:6]
                rays_d = synth_input[:, 6:]

                pred_real, pred_imag = self.renderer.render_channel(tx_o, rays_o, rays_d)

                bs = pred_real.shape[0]
                all_real[offset:offset + bs] = pred_real.cpu()
                all_imag[offset:offset + bs] = pred_imag.cpu()
                offset += bs

        # Reshape to [n_pos, n_gateways] then interleave real/imag columns
        all_real = all_real.view(n_pos, n_gateways).numpy()
        all_imag = all_imag.view(n_pos, n_gateways).numpy()

        cols = []
        col_names = []
        for i in range(n_gateways):
            cols.append(all_real[:, i])
            cols.append(all_imag[:, i])
            col_names += [f'real_g{i+1:02d}', f'imag_g{i+1:02d}']

        result = np.column_stack(cols)
        out_path = os.path.join(self.logdir, self.expname, 'synthetic_data.csv')
        os.makedirs(os.path.join(self.logdir, self.expname), exist_ok=True)
        np.savetxt(out_path, result, delimiter=',', header=','.join(col_names), comments='')
        self.logger.info("Saved synthetic synthetic data (%d rows, %d cols) to %s",
                         n_pos, len(col_names), out_path)
