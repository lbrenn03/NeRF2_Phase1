...
def split_dataset(datadir, ratio=0.8, dataset_type='rfid'):
    ...
    if dataset_type == "rfid":
        ...
        rssi_dir = os.path.join(datadir, 'features_rssi_averaged_split_ant.csv')
...
class BLE_dataset(Dataset):
    ...
    def __init__(self, datadir, indexdir, scale_worldsize=1) -> None:
        ...
        tx_pos_dir = os.path.join(datadir, 'labels_split_ant.csv')
        # self.gateway_pos_dir = os.path.join(datadir, 'gateway_position.yml')
        self.gateway_pos_dir = os.path.join(datadir, 'tx_pos.yml')
        # self.rssi_dir = os.path.join(datadir, 'gateway_rssi.csv')
        self.rssi_dir = os.path.join(datadir, 'channel_mul_nPck_reshaped.npy')
        ...
        # OLD LINE (for CSVs): self.rssis = torch.tensor(pd.read_csv(self.rssi_dir).values, dtype=torch.float32)
        self.rssis = torch.tensor(np.load(self.rssi_dir), dtype=torch.float32)
    ...
    def load_data(self):
        ...
        nn_labels = torch.tensor(np.zeros((len(self), 2)), dtype=torch.float32)
...
class Absolute_Channel_Dataset(Dataset):
    """ble dataset class
    """
    def __init__(self, datadir, indexdir, scale_worldsize=1) -> None:
        super().__init__()
...
class A_B_Dataset(Dataset):
    ...
    def __init__(self, datadir, indexdir, scale_worldsize=1) -> None:
        ...
        self.gateway_pos_dir = os.path.join(datadir, 'tx_pos.yml')
        # self.rssi_dir = os.path.join(datadir, 'gateway_rssi.csv')
        self.rssi_dir = os.path.join(datadir, 'features_channel_div_reshaped.npy')
        self.dataset_index = np.loadtxt(indexdir, dtype=int)
        self.beta_res, self.alpha_res = 9, 36  # resulution of rays
...
class Synthetic_dataset(Dataset):
    ...
    def __init__(self, datadir, indexdir, scale_worldsize=1) -> None:
        ...
        # self.rssi_dir = os.path.join(datadir, 'gateway_rssi.csv')
        self.rssi_dir = os.path.join(datadir, 'features_channel_div_reshaped.npy')
        # print(f'=====> running this code! datadir: {datadir}, indexdir: {indexdir}')
        self.dataset_index = np.loadtxt(indexdir, dtype=int)
        self.beta_res, self.alpha_res = 9, 36  # resulution of rays
...
class Synthesize_dataset(Dataset):
    """Dataset for synthesize mode: loads TX positions from labels_split_ant.csv
    and generates inputs for all positions x gateways. No ground-truth channel
    data is required — this is used to generate new synthetic channel predictions.
    """
    def __init__(self, datadir, scale_worldsize=1, tx_pos_file='labels_split_ant.csv') -> None:
        super().__init__()
        self.datadir = datadir
        tx_pos_dir = os.path.join(datadir, tx_pos_file)
        self.gateway_pos_dir = os.path.join(datadir, 'tx_pos.yml')
        ...
        # load all TX positions (no train/test split — all rows are synthesis targets)
        self.tx_poses = torch.tensor(pd.read_csv(tx_pos_dir).values, dtype=torch.float32)
        self.tx_poses = self.tx_poses / scale_worldsize
        self.n_pos = len(self.tx_poses)

        self.nn_inputs = self.load_data()

    def load_data(self):
        """Build model inputs for every (position, gateway) pair in order:
        pos0_gw0, pos0_gw1, ..., pos0_gwN, pos1_gw0, ...
        """
        input_dim = 3 + 3 + 3 * self.alpha_res * self.beta_res
        nn_inputs = torch.zeros((self.n_pos * self.n_gateways, input_dim), dtype=torch.float32)
        gateways_ray_o, gateways_rays_d = self.gen_rays_gateways()

        for i_pos, tx_pos in enumerate(tqdm(self.tx_poses, desc="Building synthesize dataset")):
            for i_gw in range(self.n_gateways):
                idx = i_pos * self.n_gateways + i_gw
                gateway_ray_o = gateways_ray_o[i_gw].view(-1)    # [3]
                gateway_rays_d = gateways_rays_d[i_gw].view(-1)  # [n_rays * 3]
                nn_inputs[idx] = torch.cat([tx_pos.view(-1), gateway_ray_o, gateway_rays_d], dim=-1)
    ...
    def gen_rays_gateways(self):
        ...
        r_d = torch.stack([x, y, z], axis=0).T  # [beta_res*alpha_res, 3]
        r_d = r_d.expand([self.n_gateways, self.beta_res * self.alpha_res, 3])
        r_o = self.gateway_pos.unsqueeze(1)  # [n_gateways, 1, 3]
        return r_o.contiguous(), r_d.contiguous()

    def __len__(self):
        return self.n_pos * self.n_gateways

    def __getitem__(self, index):
        return self.nn_inputs[index], torch.zeros(2)  # dummy label


dataset_dict = {
    "rfid": Spectrum_dataset, 
    "ble": BLE_dataset, 
    "mimo": CSI_dataset, 
    "synthesize": Synthesize_dataset, 
    "absolute_channel": Absolute_Channel_Dataset,
    "a_b": A_B_Dataset,
}
