...
class NeRF2_Runner():
    ...
    def eval_network_rssi(self):
        ...
        # ---- NEW: open error log file ----
        err_path = os.path.join(self.logdir, self.expname, "error_vs_iter.txt")
        f_err = open(err_path, "w")
        f_err.write("iter,median_error\n")   # header
    
        with torch.no_grad():
            with open(os.path.join(self.logdir, self.expname, "result.txt"), 'w') as f:
                for it, (test_input, test_label) in enumerate(self.test_iter):
                    test_input, test_label = test_input.to(self.devices), test_label.to(self.devices)
                    ...
                    med = torch.median(error).item()
                    self.logger.info("Median error:%.2f", med)
    
                    # ---- NEW: log median error per iteration ----
                    f_err.write(f"{it},{med:.6f}\n")
        ...
        # ---- NEW: automatically call plot helper ----
        plot_error_vs_iter(err_path)
    ...
    def plot_error_vs_iter(err_path):
        print("Here");
        """
        Create a plot from error_vs_iter.txt.
        """
    
        data = np.loadtxt(err_path, delimiter=",", skiprows=1)
        iters = data[:, 0]
        errors = data[:, 1]
        ...
        save_path = os.path.join(os.path.dirname(err_path), "error_plot.png")
