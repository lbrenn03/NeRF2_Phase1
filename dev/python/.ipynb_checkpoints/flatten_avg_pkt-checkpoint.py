...
def build_feature_matrix(groups):
    """
    Returns:
        X : (N rxpos, 3 txpos, 4 sym, 2 ant) complex
    ...
    for rx_id in sorted(groups.keys()):
        ...
        X.append(np.stack(tx_features, axis=0))  # (3, 4, 2)
        y.append(rx_pos * 0.6096) # Unit conversion from ft to m
        
    X = np.nanmean(np.array(X), axis=4)
    X = np.nan_to_num(X, nan=0.0)

    return np.array(X), np.array(y)  # (N, 3, 4, 2), (N, 3)
...
def flatten_and_save(X, y): 
    ...
    N = X.shape[0]
    flat = X.reshape(N, -1)  # (N, 3*4*2) complex
    ...
    df_X.to_csv(os.path.join(output_dir, 'features_pkt_avg.csv'), index=False)
    df_y.to_csv(os.path.join(output_dir, 'labels.csv'), index=False)

    print(f"Saved X: {df_X.shape} -> features_pkt_avg.csv")
...
if __name__ == '__main__':
    ...
    print(f"X shape : {X.shape}")   # (N, 3, 4, 2)
