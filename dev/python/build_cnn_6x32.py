...
# to RUN: python build_cnn_6x32.py ../../Pilot_Data_1_processed
...
def build_feature_matrix(groups):
    """
    Returns:
        X : (N, 3, 4, 2, target_packets) complex
        y : (N, 3) float  [rx_pos]
    """
    X, y = [], []
...
def flatten_and_save(X, y): 
    ...
    df_X.to_csv(os.path.join(output_dir, 'features_ab.csv'), index=False)
    df_y.to_csv(os.path.join(output_dir, 'labels.csv'), index=False)

    print(f"Saved X: {df_X.shape} -> features_ab.csv")
    print(f"Saved y: {df_y.shape} -> labels.csv")

def save_cnn_dataset(X, y):
    ...
    X_cnn = np.stack([X.real, X.imag], axis=1)   # (N,2,6,32)
    
    np.save("X_cnn_6x32.npy", X_cnn.astype(np.float32))
    np.save("y_cnn_6x32.npy", y.astype(np.float32))
    
    print("Saved:", X_cnn.shape, y.shape)
